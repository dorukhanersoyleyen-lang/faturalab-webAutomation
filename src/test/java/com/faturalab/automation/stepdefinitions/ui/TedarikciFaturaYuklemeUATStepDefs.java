...
public class TedarikciFaturaYuklemeUATStepDefs {
    ...
    public void excelDosyaSec() {
        try {
            String path = UploadTestDataPaths.bulkExcelOrCsv();
            getPage().uploadFileInDialog(path);
            log.info("Toplu yukleme dosyasi secildi: {}", path);
        } catch (Exception e) {
            log.warn("Excel/CSV dosya yolu cozulemedi: {}", e.getMessage());
        }
    }
    ...
    @And("toplu yükleme yükle butonuna tıklanır")
    public void topluYuklemeYukleTikla() {
        boolean clicked = getPage().clickKaydet();
        log.info("Toplu yükleme Yükle/Kaydet: {}", clicked);
    }
    ...
    @And("hazırlanan CSV dosyası seçilir")
    public void csvDosyaSec() {
        try {
            String path = UploadTestDataPaths.bulkCsv();
            getPage().uploadFileInDialog(path);
            log.info("CSV dosyasi secildi: {}", path);
        } catch (Exception e) {
            log.warn("CSV dosya yolu cozulemedi: {}", e.getMessage());
        }
    }

    @And("örnek XML veya PDF fatura dosyası seçilir")
    public void ornekXmlVeyaPdfSec() {
        try {
            String path = UploadTestDataPaths.eInvoiceXml();
            getPage().uploadFileInDialog(path);
            log.info("Ornek XML secildi: {}", path);
        } catch (Exception e) {
            try {
                String pdf = UploadTestDataPaths.samplePdf();
                getPage().uploadFileInDialog(pdf);
                log.info("XML yok, PDF secildi: {}", pdf);
            } catch (Exception e2) {
                log.warn("Ornek dosya yuklenemedi: {}", e2.getMessage());
            }
        }
    }

    @And("örnek görüntü dosyası seçilir")
    public void ornekGorselSec() {
        try {
            String path = UploadTestDataPaths.tinyPng();
            getPage().uploadFileInDialog(path);
            log.info("Ornek PNG secildi: {}", path);
        } catch (Exception e) {
            log.warn("PNG secilemedi: {}", e.getMessage());
        }
    }
