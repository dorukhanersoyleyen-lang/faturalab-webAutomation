...
public class AliciIhaleUATStepDefs {
    ...
    public void aliciAnaSayfasinaGidilir() {
        getPage().navigateToBuyerHome();
    }

    @And("alıcı ana sayfasında aşağı kaydırılır")
    public void aliciAnaSayfaAsagiKaydir() {
        getPage().scrollBuyerDashboardToBottom();
    }

    @And("alıcı ana sayfasında {string} butonuna tıklanır")
    public void aliciAnaSayfaButonTikla(String etiket) {
        String low = etiket.toLowerCase();
        if (low.contains("hızlı") || low.contains("hizli") || low.contains("teklif")) {
            boolean ok = getPage().clickHizliTeklifAlOnBuyerHome();
            if (!ok) {
                log.warn("Ana sayfada '{}' bulunamadı — önkoşul fatura veya konum farklı olabilir.", etiket);
            }
        } else {
            getPage().clickJsButtonPublic(etiket);
        }
    }
    ...
    public void aliciHizliTeklifModalAcilmali() {
        CompanyQuickOfferPage quick = new CompanyQuickOfferPage(DriverManager.getDriver());
        try {
            Thread.sleep(600);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
        boolean modal = quick.isModalOpen();
        if (!modal) {
            log.warn("FL-009: Modal görünmüyor. Önkoşul: yeterli fatura (iş kuralına göre, örn. 10 adet) yüklü olmalı.");
        }
        Assert.assertTrue(modal,
                "Ana sayfadaki Hızlı Teklif Al sonrası vaadin dialog görünür olmalı. URL: "
