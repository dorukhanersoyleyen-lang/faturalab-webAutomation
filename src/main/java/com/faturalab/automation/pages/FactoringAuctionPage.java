 ...
 *
 * Akış:
 *  1. Finansman kullanıcısı "Aktif İhaleler" / "İhale Listesi" ekranına gider
 *  2. Son aktif ihale için "Teklif Ver" / "GÖZAT" butonuna tıklar
 *  3. Teklif formunu doldurur (faiz oranı, teklif tutarı)
 *  4. "Kaydet" / "Teklif Ver" butonuna tıklar
 */
public class FactoringAuctionPage extends BasePageObject {
    ...
    public void navigateToActiveAuctions() {
        try {
            WebElement menu = new WebDriverWait(
    ...
    /**
     * Listede ilk aktif ihale için teklif formunu açar.
     * "Teklif Ver" / "GÖZAT" butonuna tıklar.
     */
    public void openOfferFormForFirstAuction() {
        try {
            // JS ile ilk teklif ver / gözat butonuna tıkla
    ...
    /**
     * Teklif formunu doldurur.
     *
     * @param faizOrani   Faiz oranı (%)
     * @param teklifTutari Teklif tutarı (TRY)
     */
    ...
    /**
     * Açık {@code vaadin-dialog-overlay} içinde günlük teklif / faiz alanını doldurur (Vaadin 24; label veya ilk sayısal alan).
     *
     * @param ratePercent örn. {@code 3.5} — {@code uat.factoring.offer.rate} ile override edilebilir
     */
    public void fillGunlukTeklifDialogRateIfPresent(String ratePercent) {
        try {
            try {
                new WebDriverWait(driver, java.time.Duration.ofSeconds(12)).until(
                        ExpectedConditions.presenceOfElementLocated(
                                By.cssSelector("vaadin-dialog-overlay, vaadin-number-field, vaadin-text-field")));
            } catch (Exception waitEx) {
                log.debug("Teklif dialog alanı beklenirken: {}", waitEx.getMessage());
            }
            Boolean ok = (Boolean) ((JavascriptExecutor) driver).executeScript(
                    ...
                    "var root = overlay || document.body;" +
                    "var fields = root.querySelectorAll('vaadin-number-field, vaadin-integer-field, vaadin-text-field');" +
                    "for (var f of fields) {" +
                    "  var lbl = (f.getAttribute && f.getAttribute('label') || '').toLowerCase();" +
                    "  var inp = f.querySelector('input');" +
                    "  if (!inp) continue;" +
                    "  if (lbl.includes('faiz') || lbl.includes('oran') || lbl.includes('rate') || lbl.includes('%')) {" +
                    "    inp.focus(); inp.value = rate;" +
                    "    inp.dispatchEvent(new Event('input', {bubbles: true}));" +
                    "    inp.dispatchEvent(new Event('change', {bubbles: true}));" +
                    "    return true;" +
                    "  }" +
                    "}" +
                    "for (var f2 of fields) {" +
                    "  var inp2 = f2.querySelector('input');" +
                    "  if (inp2 && !inp2.value) {" +
                    "    inp2.focus(); inp2.value = rate;" +
            ...
            if (Boolean.TRUE.equals(ok)) {
                log.info("Günlük teklif dialog faiz alanı: {}", ratePercent);
            } else {
                fillOfferForm(ratePercent, "");
            }
            Thread.sleep(400);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } catch (Exception e) {
            log.warn("Günlük teklif dialog doldurma: {}", e.getMessage());
        }
    }

    /**
     * "Kaydet" / "Teklif Ver" butonuna tıklar.
     */
    ...
    /**
     * Teklifin başarıyla oluşturulduğunu doğrular.
     */
    ...
    /**
     * Teklifin limit ve fiyat kriterlerine uygun olup olmadığını kontrol eder.
     * Hata mesajı yoksa kriter uyumlu kabul edilir.
     */
